...
收起成窄栏（rail）时自动变成三个圆形图标按钮，顺序不变。

行列几何完全对齐「设置」那一行：42px 行高、12px 圆角、8px 间距、14px 字号、同样的 hover 底色与 `--dsw-*` 令牌；窄栏下同样是 36px 圆形。这不是"看着差不多"——测试会把本插件的样式声明与已安装的官方 `ui-settings-general` 样式逐条比对（见下）。
